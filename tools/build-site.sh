...
write_page() (
  ...
  cleanup_page_files() {
    ...
    [ -n "$final_rendered" ] && rm -f "$final_rendered"
  ...
  final_rendered=$(mktemp "$BUILD/.page-final-rendered.XXXXXX")
  ...
  expand_template_file "$rendered" > "$final_rendered"

  signature_file=$(page_signature_path "$rel_no_ext")
  mkdir -p "$(dirname -- "$signature_file")"
  current_signature=$(page_build_signature "$final_rendered" "$renderer_page" "$root_prefix")
